		...
		if (paramSet != null) {
			for (Parameter param : paramSet) {
				...
				if (param.getType() == ParameterType.INTEGER) {
					...
					JParameterSlider slider = new JParameterSlider((int) Math.round(param.getMinValue()),
							(int) Math.round(param.getMaxValue()), (int) Math.round(param.getValue()), param, currValue);
					add(slider);
					...
					JDecimalSlider slider = new JDecimalSlider(param.getMinValue(), param.getMaxValue(), param.getValue(),
							DECIMAL_SPOTS, param, currValue);
					add(slider);
