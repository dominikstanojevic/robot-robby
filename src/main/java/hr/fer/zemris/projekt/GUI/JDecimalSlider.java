...
public class JDecimalSlider extends JParameterSlider {
	...
	public JDecimalSlider(double min, double max, double value, int decimalSpots, Parameter parameter,
			JLabel lCurrentValue) {
		super(parameter, lCurrentValue);

		if (decimalSpots < 0)
			throw new IllegalArgumentException("Number of decimal spots must be greater of equal to zero.");
