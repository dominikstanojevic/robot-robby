...
public class ProgramTreeParser {
	...
	public Node parse() throws ParsingException {
		...
		while (currIndex < tree.length() && tree.charAt(currIndex) != '(' && tree.charAt(currIndex) != ')'
				&& tree.charAt(currIndex) != ',') {
	 ...
	 * @param nodeName name of the node to be parsed
	 * @return node
	 * @throws ParsingException if no node with the given name exists
	 */
